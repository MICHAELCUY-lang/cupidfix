...
-- PWA Installation SQL Script for Cupid
-- Run this script to add the necessary database tables for PWA functionality
...
CREATE TABLE IF NOT EXISTS push_subscriptions (
    ...
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    UNIQUE KEY user_endpoint (user_id, endpoint(250))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Table for storing user app installations
CREATE TABLE IF NOT EXISTS app_installations (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    device_type VARCHAR(50) NOT NULL, -- 'android', 'ios', 'windows', 'macos', 'linux', etc.
...
-- Add PWA preferences to user profiles (optional)
ALTER TABLE profiles
ADD COLUMN pwa_notifications_enabled TINYINT(1) DEFAULT 1,
ADD COLUMN pwa_message_notifications TINYINT(1) DEFAULT 1,
ADD COLUMN pwa_match_notifications TINYINT(1) DEFAULT 1,
ADD COLUMN pwa_system_notifications TINYINT(1) DEFAULT 1;
...
CREATE TABLE IF NOT EXISTS notification_logs (
    ...
    notification_type VARCHAR(50) NOT NULL, -- 'message', 'match', 'system', etc.
    related_id INT NULL, -- ID of related item (message_id, match_id, etc.)
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Insert default value for VAPID keys (you'll need to replace these with your actual keys)
...
-- Insert placeholder for VAPID keys (you'll need to generate and replace these)
INSERT INTO app_settings (setting_key, setting_value, description)
VALUES 
('vapid_public_key', 'your_public_key_here', 'VAPID public key for Web Push notifications'),
('vapid_private_key', 'your_private_key_here', 'VAPID private key for Web Push notifications');

-- Add column for offline data preferences
ALTER TABLE users
ADD COLUMN offline_data_enabled TINYINT(1) DEFAULT 1;
